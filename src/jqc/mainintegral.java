...
public class mainintegral {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                for (int k = 0; k < geo.numProton.length; k++) {
                    ak1++;
                    while (Thread.activeCount() > Integer.parseInt(kernel.a.thr_nri)) {
                    }
    ...
    public void two(String name) throws InterruptedException {
        ...
        for (int i = 0; i < n; i++) {
              System.out.println((int) ((iteration / (n * n)) * 100.0));
            for (int j = 0; j < n; j++) {
                String int_p=String.valueOf((iteration++ / (n * n)) * 100.0);
                int panp=Math.min(5, int_p.length()-1);
                //kernel.a.int_p.setText(int_p.substring(0,panp));
                //kernel.a.proses_int.setValue((int) ((iteration / (n * n)) * 100.0));
                for (int k = 0; k < n; k++) {
                    for (int l = 0; l < n; l++) {
                        if (cek[i][j][k][l] == 0) {
                            ak++;
                            while (Thread.activeCount() > Integer.parseInt(kernel.a.thr_eri)) {
                            }
