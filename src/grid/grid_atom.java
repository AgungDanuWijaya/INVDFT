...
public class grid_atom {
	...
	public HashMap<Integer, double[]> rungrid(double batom, double[] R,main_function kernel) {
		HashMap<Integer, double[]> point = new HashMap<>();
		lebedev a = new lebedev();
                double fitnes=Double.parseDouble(kernel.a.dens);
		double[][] grid_params = LegendreGrid(batom,fitnes);
		int i = 0;
	...
	public double[][] LegendreGrid(double batom, double fineness) {
		bragg a = new bragg();
